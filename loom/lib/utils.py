...
from ..common.config import (
    DUCKDB_MIGRATION_DOWNGRADES,
    DUCKDB_MIGRATION_UPGRADES,
    MIGRATION_DOWNGRADES,
    MIGRATION_UPGRADES,
)
...
def get_duckdb_migrations(direction: str) -> List[Migration]:
    migrations = []
    migration_path = (
        DUCKDB_MIGRATION_UPGRADES if direction == "up" else DUCKDB_MIGRATION_DOWNGRADES
    )
    files = sorted(os.listdir(migration_path))
    for file in files:
        if file.endswith(".sql"):
...
def get_duckdb_upgrade_migrations() -> List[Migration]:
    return get_duckdb_migrations("up")


def get_duckdb_downgrade_migrations() -> List[Migration]:
    return get_duckdb_migrations("down")
